...
class weather_data_handler_t
{
	...
	auto on_new_weather(const restinio::request_handle_t& req, rr::route_params_t )
	{
        ...
        try
        {
            auto newWeatherReg = json_dto::from_json<weather_data_t>(req->body());
            m_weather_data.emplace_back(json_dto::from_json< weather_data_t >( req->body() ) );

            resp.set_body("New weather registration added successfully");
        }
        catch (const std::exception &ex)
        {
            mark_as_bad_request(resp);
            //resp.set_body("Error: " + std::string(ex.what()));
        }
...
auto server_handler( weather_data_collection_t & weather_collection )
{
	...
	router->http_post( "/", by( &weather_data_handler_t::on_new_weather ) );
